...
```{r asdasdas_ASDasd}
# Consider the DEGs that you obtained in the common part, and the two tissues that were compared: how many DEGs are "elevated" in one or the other individual?

# Filter DEGs for individual-specific and individual-elevated expression
individual_specific_genes <- names(expression_classes[grep("individual_specific_", expression_classes)])
individual_elevated_genes <- names(expression_classes[grep("individual_elevated_", expression_classes)])
individual_specific_degs <- individual_specific_genes[individual_specific_genes %in% rownames(top_genes$table)]
individual_elevated_degs <- individual_elevated_genes[individual_elevated_genes %in% rownames(top_genes$table)]
individual_specific_degs_count <- length(individual_specific_degs)
individual_elevated_degs_count <- length(individual_elevated_degs)
print(paste("Number of individual-specific DEGs:", individual_specific_degs_count))
print(paste("Number of individual-elevated DEGs:", individual_elevated_degs_count))
